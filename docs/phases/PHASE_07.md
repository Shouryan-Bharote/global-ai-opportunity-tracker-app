...
| 2026-08-23 | Migrated Flutter domain model from mock `EventModel` to canonical Firestore `OpportunityModel` | `lib/features/opportunities/models/opportunity_model.dart`, `lib/features/opportunities/repositories/firestore_opportunity_repository.dart`, `lib/features/opportunities/providers/opportunities_provider.dart`, `lib/features/opportunities/screens/opportunity_details_screen.dart`, `lib/features/explore/`, `lib/features/home/`, `lib/features/schedule/`, `test/opportunity_model_test.dart` | Migrated data layer to consume real Firestore `opportunities` collection directly. Defensive date parsing (Timestamps + ISO strings), null safety, enum fallbacks, search and filter bindings across Explore, Home, and Schedule. All 7 unit/widget tests passing cleanly. |
